...
for smoothingFactor = [2]                                        % this smooths between depth labels 
    for dataFactor = [90]                                        % for nice smooth along a single line, 15, 400, 40, 0 works well
        for horzNeighborMaskWeight = [2]
            for verticalNeighborMaskWeight = [2]
                ...
                minLabel = 60;
                maxLabel = 150;
                f_l = 6.1762;        % left camera is ir
                f_r = 6.1762;         % right camera is RGB
                        ...
                        for i = 1:numCols
                            for j = 1: numLabels
                                ...
                                %if i + disparity <= numCols
                                    m = (f_r/f_l)*(Z+d)/(Z);
                                    leftPixel_1 = i;
                                    x_l = (leftPixel_1-numCols/2)*.006;
                                    x_r = (-b*f_r*f_l+f_r*x_l*Z+f_r*x_l*d)/(Z*f_l);
                                    rightPixel_1 = numCols/2+x_r/.006;        % this is a fractional pixel
                                    %rp(j) = rightPixel_1;
                                    if (rightPixel_1 < 1)
                                        dataCost(j, site) = 10;
